...
export default class MathFighterScene extends Phaser.Scene {
    ...
    update() {
        ...
        if (this.correctAnswer === true && !this.enemyAttack) {
            this.enemy.anims.play('enemy-attack', true);
            this.time.delayedCall(500, () => {
                this.createSlash(this.player.x - 60, this.player.y, 2, -600, true);
            });
    ...
    gameStart() {
        ...
        this.player.anims.play('player-standby', true);
        this.enemy.anims.play('enemy/standby', true);
        this.resultText = this.add.text(this.gameHalfWidth, 250, '0', { fontSize: '32px', color: '#000' });
        this.questionText = this.add.text(this.gameHalfWidth, 200, '0', { fontSize: '32px', color: '#000' });
    ...
    createButtons() {
        const startposY = this.scale.height - 232;
        const widthDiff = 131;
    ...
    createSlash(x, y, frame, velocity, flip = false, attacker = null) {
        this.slash.setPosition(x, y)
            ...
            .setData("attacker", attacker);
    }
    spriteHit(slash,sprite) {
        const attacker = slash.getData("attacker")
        if (sprite === attacker) {
            return
        }
        slash.x = 0
